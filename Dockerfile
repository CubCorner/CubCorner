...
COPY --from=node-builder /root/.cache/node /home/cubcorner/.cache/node
...
ARG HOST_UID=1000
ARG HOST_GID=1000
RUN addgroup --gid ${HOST_GID} cubcorner && \
  adduser -S --shell /bin/sh --uid ${HOST_UID} cubcorner && \
  addgroup cubcorner wheel && \
  echo "cubcorner ALL=(ALL) NOPASSWD:ALL" >> /etc/sudoers
